...
export const staysService = {
  ...
  async createStay(payload: {
    ...
    const newStayId = stayId as string

    const { error: featuresError } = await supabase.rpc('enable_all_features_for_stay', {
      p_stay_id: newStayId,
    })
  ...
  async updateStay(
    ...
    const nextTitle =
      payload.title === undefined
        ? undefined
        : requireNonEmpty(payload.title, 'Nom du séjour')
    ...
    const updatePayload = omitUndefined({
      ...
      location_name:
        payload.locationName === undefined
          ? undefined
          : emptyToNull(payload.locationName ?? ''),
      location_address:
        payload.locationAddress === undefined
          ? undefined
          : emptyToNull(payload.locationAddress ?? ''),
      location_url:
        payload.locationUrl === undefined
          ? undefined
          : emptyToNull(payload.locationUrl ?? ''),
    })
    ...
    const { error } = await supabase.rpc('archive_stay', {
      p_stay_id: stayId,
    })
  ...
  async deleteStay(stayId: string) {
    ...
    const { error } = await supabase.rpc('delete_stay', {
      p_stay_id: stayId,
    })
