...
export function SettingsPageClient({ stay, settings, features, isOwner }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <form onSubmit={handleSaveStay} className="flex flex-col gap-4 rounded-xl border border-neutral-200 bg-white p-5">
        ...
        <Field label="Nom du séjour"><input required value={title} onChange={e => setTitle(e.target.value)} className="input" /></Field>
        <Field label="Lieu"><input value={locationName} onChange={e => setLocationName(e.target.value)} className="input" /></Field>

        <div className="grid grid-cols-2 gap-3">
          <Field label="Début"><input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="input" /></Field>
          <Field label="Fin"><input type="date" value={endDate} min={startDate} onChange={e => setEndDate(e.target.value)} className="input" /></Field>
        </div>

        <div className="flex flex-col gap-2">
          <label className="text-xs font-medium text-neutral-600">Couleur du séjour</label>
          <div className="flex flex-wrap gap-2">
            {STAY_COLOR_OPTIONS.map(c => <button key={c.id} type="button" onClick={() => setPrimaryColor(c.value)} className="h-8 w-8 rounded-full transition-transform hover:scale-110" style={{ backgroundColor: c.value, outline: primaryColor === c.value ? `2px solid ${c.value}` : 'none', outlineOffset: '2px' }} title={c.label} />)}
          </div>
        </div>

        <button type="submit" disabled={saving} className="rounded-lg bg-[var(--stay-primary)] py-2.5 text-sm font-medium text-[var(--stay-primary-text)] hover:opacity-90 disabled:opacity-50">{saving ? 'Enregistrement…' : 'Enregistrer'}</button>
      </form>

      <div className="rounded-xl border border-neutral-200 bg-white p-5">
        <h3 className="mb-4 text-sm font-medium text-neutral-700">Options activées</h3>
        {featureRows.length === 0 ? <p className="text-sm text-neutral-500">Aucune option avancée activée pour le moment.</p> : (
          <div className="divide-y divide-neutral-100">
            {featureRows.map(f => (
              <div key={f.feature_key} className="flex items-center justify-between gap-4 py-3">
                ...
                <button type="button" onClick={() => toggleFeature(f.feature_key, !f.is_enabled)} className={`rounded-full px-3 py-1 text-xs font-medium ${f.is_enabled ? 'bg-green-50 text-green-700' : 'bg-neutral-100 text-neutral-500'}`}>{f.is_enabled ? 'Actif' : 'Inactif'}</button>
              </div>
            ))}
          </div>
        )}
      ...
      {isOwner && (
        <div className="rounded-xl border border-red-200 bg-white p-5">
          ...
          <button type="button" onClick={() => void handleArchive()} className="rounded-lg border border-red-200 px-4 py-2 text-sm text-red-600 hover:bg-red-50">Archiver le séjour</button>
        </div>
      )}
    </div>
  )
